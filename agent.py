...
class agent():
	...
	def __init__(self, lr=0.001, s_size=170, a_size=2):
		...
		hidden2 = slim.fully_connected(hidden, 50, biases_initializer=None,activation_fn=tf.nn.relu, weights_initializer=tf.ones_initializer())
		output = slim.fully_connected(hidden, a_size, biases_initializer=None, activation_fn=tf.nn.sigmoid, weights_initializer=tf.ones_initializer())
